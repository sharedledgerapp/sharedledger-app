...
- **Communication**: 2-tab messaging layout (Messages | Notes). Messages tab shows a WhatsApp-style conversation list with Sage AI row (pinned) + Group Chat row (unread badge + last message preview). Notes tab has Personal Notes section (private, only visible to owner) and Shared Notes section (group-only). Personal notes support "Ask Sage" (opens Sage with note content pre-filled) and "Share to Group" (posts to group chat). New `personal_notes` DB table (no family_id — private per user).
